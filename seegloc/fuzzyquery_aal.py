...
def lookup_fsl(coords_mm: Tuple[float, float, float], atlas_name: str) -> str:
    fsl_path = os.environ.get('FSLDIR', None)
    p = subprocess.Popen([
        os.path.join(fsl_path, 'bin', 'atlasquery'), '-a', atlas_name, '-c',
        f'{coords_mm[0]:f},{coords_mm[1]:f},{coords_mm[2]:f}'
    ],
    ...
    if atlas_name == 'Harvard-Oxford Cortical Structural Atlas':
        # parse output
        parse = re.split(r', (\d{1,2}%)', out)

        # join every two array items
        parse = [parse[0]] + [
            parse[i] + parse[i + 1] for i in range(1, len(parse), 2)
        ]
        out = parse

    if out == 'No label found!':
        out = None
